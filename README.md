...
### ✨ 核心特色
- **Git 自動索引與雙主機支援**：智慧偵測並同時支援 **Gitea** 與 **GitHub**，自動抓取 `SKILL.md` 並生成文檔。
- **自訂標籤管理 (Tags Editing)**：註冊技能時可編輯自訂標籤，並與文件內容自動去重合併，提升索引精準度。
- **標籤雲探索 (Tag Cloud)**：全新的「標籤篩選」模式，支援多選標籤與標籤計數顯示，讓搜尋更直覺。
